    ...
    const handleSave = async (object: any) => {
        ...
        for (const key in props.objectKeys) {
          ...
          if (Object.prototype.hasOwnProperty.call(props.objectKeys, key)) {
            ...
            Object.assign(objectData, {[key]: object[key]});
            continue;
          }
          // categories
          if (key === 'categories') {
        ...
        console.log("formDataformDataformData", formData, objectData)
        try {
          if (object.id) {
            // Se o objeto já possui um ID, é uma atualização (método PUT)
            await api.put(route + `/${object.id}`, objectData, {
              // headers: {
            ...
            await api.post(route, objectData, {
              // headers: {
              //   'Content-Type': 'multipart/form-data',
              // },
            });
